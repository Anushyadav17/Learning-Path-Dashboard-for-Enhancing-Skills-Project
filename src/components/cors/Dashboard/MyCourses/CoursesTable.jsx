...
export default function CoursesTable({ courses, setCourses }) {
  ...
  return (
    <>
      <Table className="rounded-xl  my-10 flex flex-col gap-y-6 rounded-md border-[1px] border-pure-greys-50 bg-white shadow-md ">
        <Thead >
        ...
        <Tbody>
          {courses?.length === 0 ? (
            ...
            courses?.map((course) => (
              <Tr
                key={course?._id}
                className="flex gap-x-10  px-6 py-0 pb-3 gap-4"
              >
                <Td colSpan={1}  className="flex flex-1 gap-x-4 px-2">
                  <img
                    src={course?.thumbnail}
                    alt={course?.courseName}
                    className="md:h-[148px] md:w-[250px] aspect-video rounded-lg object-cover"
                  />
                ...
                <Td className="text-sm font-medium text-black mb-5 py-4">
                  ₹{course.price}
                </Td>
                <Td className="text-sm font-medium text-black py-4">
                  <button
                    disabled={loading}
                    onClick={() => {
